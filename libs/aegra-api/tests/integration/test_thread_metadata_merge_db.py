...
class PausedBeforeWrite:
    ...
    async def execute(self, *args: Any, **kwargs: Any) -> Any:
        if "pg_advisory_xact_lock" not in str(args[0]):
...
async def test_concurrent_patches_keep_both_sets_of_keys(sessions: SessionFactory) -> None:
    ...
    async with sessions() as slow, sessions() as fast:
        ...
        fast_patch = asyncio.create_task(
            update_thread(thread_id, ThreadUpdate(metadata={"from_fast": "two"}), user=_user(), session=fast)
        )
        await asyncio.sleep(0.1)
        assert not fast_patch.done()  # Both writers share the cleanup/admission gate.
        resume.set()
        await asyncio.wait_for(asyncio.gather(slow_patch, fast_patch), timeout=_WAIT_TIMEOUT)
